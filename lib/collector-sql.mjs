...
export const BULK_UPSERT_SOURCE_COVERAGE_SQL = `INSERT INTO collector_source_coverage
  (source_id, complete, collected_at)
  SELECT
    json_extract(incoming.value, '$.sourceId'),
    CAST(COALESCE(json_extract(incoming.value, '$.complete'), 0) AS INTEGER),
    json_extract(incoming.value, '$.collectedAt')
  FROM json_each(?) AS incoming
  ...
  ON CONFLICT(source_id) DO UPDATE SET
    ...
    collected_at = excluded.collected_at
  WHERE collector_source_coverage.collected_at < excluded.collected_at`;
...
export const BULK_DELETE_SUCCESS_EVENTS_SQL = `DELETE FROM collector_events
  WHERE source_id IN (
    ...
    WHERE json_extract(incoming.value, '$.status') = 'success'
      AND CAST(json_extract(incoming.value, '$.complete') AS INTEGER) = 1
  )`;
